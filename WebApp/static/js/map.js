...
function getDefaultMarkerIcon(svgColor) {
...
class Marker {
    constructor(id, name, position, icon, map, locationWindow, windowContent, setIsWindowOpen) {
        this.id = id;
        this.name = name;
    ...
    async addMarker() {
        const marker = new AdvancedMarkerElement({
            ...
            content: this.icon.cloneNode(true),
            title: this.name,
        });
        this.marker = marker;
    ...
    _updateMarkerIcon() {
        ...
        const pinSvg = getDefaultMarkerIcon(svgColor);
        this.marker.content = pinSvg.cloneNode(true);
    }

    async updateMarkerData() {
...
async function main() {
    ...
    const pinSvg = getDefaultMarkerIcon();

    const {locationWindow, windowContent, setIsWindowOpen} = initializeStationPanel();

    stationData.forEach(station => {
        const newMarker = new Marker(station.number, station.name, { lat: station.lat, lng: station.lng }, pinSvg, map, locationWindow, windowContent, setIsWindowOpen);
        markers.push(newMarker);
    });

    setInterval(async () => {
        await getAvailabilityData();
        markers.forEach(marker => marker.updateMarkerData());
    }, 60000);
}
